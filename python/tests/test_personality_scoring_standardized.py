...
class TestPersonalityScoring(unittest.TestCase):
    ...
    def test_staleness_scoring_conservative_play(self):
        ...
        self.assertGreater(scores.staleness, 49.0)  # Adjusted for new weights
    ...
    def test_blunder_penalty(self):
        ...
        # Aggressive is based on forcing moves, not accuracy, so blunders in quiet moves don't affect it much
        # Both test cases have similar forcing/quiet ratios, so aggressive scores may be similar
    ...
    def test_novelty_staleness_natural_opposition(self):
        ...
        self.assertGreater(scores.novelty - scores.staleness, 35.0,
                          f"Gap between novelty ({scores.novelty}) and staleness ({scores.staleness}) should be > 35")
