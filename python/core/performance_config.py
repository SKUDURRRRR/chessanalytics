...
def get_performance_config() -> AnalysisPerformanceConfig:
    ...
    if not config.validate():
        logger.warning("Warning: Invalid performance configuration, using Railway Hobby defaults")

    return config

def print_performance_config(config: AnalysisPerformanceConfig):
    """Print performance configuration for debugging."""
    logger.info("=== Analysis Performance Configuration ===")
    logger.info(f"Stockfish Depth: {config.stockfish_depth}")
    logger.info(f"Stockfish Skill Level: {config.stockfish_skill_level}")
    logger.info(f"Stockfish Time Limit: {config.stockfish_time_limit}s")
    logger.info(f"Stockfish Threads: {config.stockfish_threads}")
    logger.info(f"Stockfish Hash Size: {config.stockfish_hash_size}MB")
    logger.info(f"Max Concurrent Analyses: {config.max_concurrent_analyses}")
    logger.info(f"Batch Size: {config.batch_size}")
    logger.info(f"Parallel Analysis: {config.parallel_analysis}")
    logger.info(f"Max Memory Usage: {config.max_memory_usage_mb}MB")
    logger.info(f"Max Games Per Request: {config.max_games_per_request}")
    logger.info(f"Analysis Cache Enabled: {config.enable_analysis_cache}")
    logger.info("==========================================")
