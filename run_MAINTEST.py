...
# Load environment variables (root + backend-specific)
project_root = Path(__file__).resolve().parent

# Root-level dotenv files
load_dotenv(dotenv_path=project_root / '.env', override=False)
load_dotenv(dotenv_path=project_root / '.env.local', override=False)

# Backend (python/) dotenv files
backend_env_dir = project_root / 'python'
load_dotenv(dotenv_path=backend_env_dir / '.env', override=False)
load_dotenv(dotenv_path=backend_env_dir / '.env.local', override=False)
    ...
    def run_frontend_tests(self) -> bool:
        ...
        try:
            ...
            if not npx_executable:
                raise FileNotFoundError("npx was not found on PATH. Please ensure Node.js is installed and npx is available.")
