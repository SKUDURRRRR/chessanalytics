...
def get_time_control_category(time_control: str) -> str:
    """Categorize time control - matches frontend logic"""
    if not time_control or time_control == '-' or time_control.lower() == 'unknown':
        return 'Unknown'
    
    tc_lower = time_control.lower()
    
    # Handle correspondence/daily games
    if 'correspondence' in tc_lower or 'day' in tc_lower or '/' in tc_lower:
        return 'Correspondence'
    
    # Parse time control (format: base+increment or just base)
    try:
        if '+' in time_control:
            base_str = time_control.split('+')[0]
        else:
            base_str = time_control
        
        base_seconds = int(base_str)
        
        # Categorize based on base time
        if base_seconds < 180:  # Less than 3 minutes
            return 'Bullet'
        elif base_seconds < 600:  # Less than 10 minutes
            return 'Blitz'
        elif base_seconds < 1800:  # Less than 30 minutes
            return 'Rapid'
        else:
            return 'Classical'
