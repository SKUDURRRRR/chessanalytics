...
-- This fixes the error: column "games_import_limit" does not exist
-- Date: 2025-11-04
...
BEGIN
    ...
    -- Get tier limits from payment_tiers table (correct column names: import_limit, analysis_limit)
...
COMMENT ON FUNCTION check_usage_limits IS 'Checks if user has exceeded their usage limits (24h rolling window). Fixed to use correct column names: import_limit and analysis_limit from payment_tiers table.';

-- Verify the function was created correctly
DO $$
BEGIN
    RAISE NOTICE 'Function check_usage_limits has been recreated successfully';
    RAISE NOTICE 'This should fix the "column games_import_limit does not exist" error';
END $$;
