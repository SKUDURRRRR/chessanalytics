...
const ECO_CODE_MAPPING: Record<string, string> = {
...
export function getOpeningNameFromECOCode(ecoCode: string): string {
  if (!ecoCode || typeof ecoCode !== 'string') {
    return 'Unknown'
  }
  
  // Clean the ECO code (remove any extra characters)
  const cleanEcoCode = ecoCode.trim().toUpperCase()
  
  // Check if it's a valid ECO code format (A00-E99)
  if (!/^[A-E]\d{2}$/.test(cleanEcoCode)) {
    return ecoCode // Return original if not a valid ECO code
  }
  
  return ECO_CODE_MAPPING[cleanEcoCode] || ecoCode
}
...
export function normalizeOpeningName(opening: string): string {
  ...
  const ecoName = getOpeningNameFromECOCode(opening)
  if (ecoName !== opening) {
    return ecoName
  }
