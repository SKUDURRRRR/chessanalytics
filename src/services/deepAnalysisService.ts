...
export async function fetchDeepAnalysis(
  ...
  const { data: games, error: gamesError } = await supabase
    .from('games')
    .select('id, user_id, platform, result, color, provider_game_id, opening, opening_family, accuracy, opponent_rating, my_rating, time_control, played_at, created_at')
    .eq('user_id', userId)
  ...
  // Fetch detailed analysis data from move_analyses table (Stockfish analysis)
  const { data: moveAnalyses, error: moveAnalysesError } = await supabase
    .from('move_analyses')
    .select('*')
    .eq('user_id', userId)
    .eq('platform', platform === 'chess.com' ? 'chess.com' : 'lichess')
  ...
  if (moveAnalysesError) {
    console.warn('Error fetching move analysis data:', moveAnalysesError)
  }
  ...
  if (analyses && analyses.length > 0) {
    averageAccuracy = analyses.reduce((sum, a) => sum + (a.accuracy || 0), 0) / analyses.length
    console.log('Using analysis data for accuracy:', { analysisCount: analyses.length, averageAccuracy })
  } else {
    averageAccuracy = games.reduce((sum, g) => sum + (g.accuracy || 0), 0) / totalGames
  ...
  // Prioritize move_analyses (Stockfish data) over game_analyses for personality scores
  const personalityScores = calculatePersonalityScoresFromGames(games, moveAnalyses || analyses || [], userId)
...
function calculatePersonalityScoresFromGames(games: any[], analyses: any[] = [], userId: string = 'unknown'): DeepAnalysisData['personalityScores'] {
  if (games.length === 0) {
    return {
      tactical: 50,
      positional: 50,
  ...
  const analyzedGames = games.filter(g => 
    g.accuracy !== null && 
  ...
  if (analyzedGames.length > 0) {
    ...
    return {
      ...
      positional: Math.round(combinedScores.positional * weight + basicScores.positional * (1 - weight)),
      aggressive: Math.round(combinedScores.aggressive * weight + basicScores.aggressive * (1 - weight)),
  ...
  console.log('Using basic estimation fallback - this will give similar scores for similar ratings!')
...
function estimatePersonalityFromBasicData(games: any[], userId: string = 'unknown'): DeepAnalysisData['personalityScores'] {
  const totalGames = games.length
  ...
  if (avgAccuracy === 0 && avgBlunders === 0 && avgMistakes === 0) {
    ...
    avgInaccuracies = Math.max(0, Math.round((100 - avgAccuracy) / 6 + (avgRating < 1200 ? 1 : 0.2)))
    ...
    const gameLengthVariation = (games.reduce((sum, g) => sum + (g.total_moves || 40), 0) / totalGames - 40) / 10
    const openingVariation = games.filter(g => g.opening && g.opening.length > 10).length / totalGames
  ...
  const aggressiveOpenings = games.filter(g => {
    ...
    return opening.includes('sicilian') || opening.includes('king\'s indian') || 
           opening.includes('dragon') || opening.includes('najdorf') ||
           opening.includes('pirc') || opening.includes('modern') ||
           opening.includes('gambit') || opening.includes('attack')
  }).length
  ...
  const endgameBase = Math.max(0, avgAccuracy - avgBlunders * 2)
  const endgame = Math.round(getSkillBasedScore(avgRating, endgameBase))
  
  const openingBase = Math.max(0, avgAccuracy - avgInaccuracies * 2)
...
function calculatePersonalityFromMoveAnalyses(moveAnalyses: any[]): DeepAnalysisData['personalityScores'] {
  ...
  const positional = moveAnalyses.reduce((sum, a) => sum + (a.positional_score || 0), 0) / totalAnalyses
  const aggressive = moveAnalyses.reduce((sum, a) => sum + (a.aggressive_score || 0), 0) / totalAnalyses
...
function calculatePersonalityFromAnalyzedGames(analyses: any[]): DeepAnalysisData['personalityScores'] {
  ...
  const avgBrilliantMoves = analyses.reduce((sum, a) => sum + (a.brilliant_moves || 0), 0) / totalAnalyses
  ...
  const avgMiddleGameAccuracy = analyses.reduce((sum, a) => sum + (a.middle_game_accuracy || 0), 0) / totalAnalyses
  ...
  const positionalBase = Math.max(0, 
    avgMiddleGameAccuracy * 0.8 + 
    Math.min(10, (avgGameLength - 30) * 0.5) +
    (100 - avgBlunders * 3)
  )
  const positional = Math.round(getSkillBasedScore(avgRating, positionalBase))
  
  // Calculate aggressive score based on aggressiveness index, material sacrifices, and opening choices
  const avgAggressiveness = analyses.reduce((sum, a) => sum + (a.aggressiveness_index || 0), 0) / totalAnalyses
  const avgSacrifices = analyses.reduce((sum, a) => sum + (a.material_sacrifices || 0), 0) / totalAnalyses
  ...
  const aggressiveBase = Math.max(0, Math.min(100, 
    avgAggressiveness * 100 + 
    avgSacrifices * 15 + 
    (aggressiveOpenings / totalAnalyses) * 20
  ))
  const aggressive = Math.round(getSkillBasedScore(avgRating, aggressiveBase))
  
  // Calculate patient score based on accuracy, low blunder rate, and endgame performance
  const avgEndgameAccuracy = analyses.reduce((sum, a) => sum + (a.endgame_accuracy || 0), 0) / totalAnalyses
  const avgInaccuracies = analyses.reduce((sum, a) => sum + (a.inaccuracies || 0), 0) / totalAnalyses
  
  // Patient: High accuracy + low blunders + good endgame + low inaccuracies
  const patientBase = Math.max(0, 
    avgAccuracy * 0.6 + 
    avgEndgameAccuracy * 0.3 - 
    avgBlunders * 2 - 
    avgInaccuracies * 0.5
  )
  const patient = Math.round(getSkillBasedScore(avgRating, patientBase))
  
  // Calculate endgame score based on endgame accuracy and conversion rate
  const endgameGames = analyses.filter(a => a.endgame_accuracy > 0)
  const endgameConversionRate = analyses.filter(a => a.result === 'win' && a.endgame_accuracy > 0).length / Math.max(1, endgameGames.length)
  
  const endgameBase = Math.max(0, 
    avgEndgameAccuracy * 0.8 + 
    endgameConversionRate * 20
  )
  const endgame = Math.round(getSkillBasedScore(avgRating, endgameBase))
  
  // Calculate opening score based on opening accuracy and variety
  const avgOpeningAccuracy = analyses.reduce((sum, a) => sum + (a.opening_accuracy || 0), 0) / totalAnalyses
  const uniqueOpenings = new Set(analyses.map(a => a.opening).filter(Boolean)).size
  const openingVariety = Math.min(20, uniqueOpenings * 2)
  
  const openingBase = Math.max(0, 
    avgOpeningAccuracy * 0.8 + 
    openingVariety
  )
  const opening = Math.round(getSkillBasedScore(avgRating, openingBase))
...
function determinePlayerLevel(rating: number, accuracy: number): DeepAnalysisData['playerLevel'] {
...
function determinePlayerStyle(scores: DeepAnalysisData['personalityScores']): DeepAnalysisData['playerStyle'] {
  const { tactical, positional, aggressive, patient } = scores
  
  if (positional > tactical && positional > aggressive) {
    return {
      category: 'positional',
      description: 'reminiscent of Anatoly Karpov and Tigran Petrosian - positional players who excel at long-term strategy',
  ...
  if (tactical > positional && tactical > aggressive) {
    return {
      category: 'tactical',
      description: 'reminiscent of Mikhail Tal and Garry Kasparov - tactical players who excel at combinations',
  ...
  if (aggressive > positional && aggressive > tactical) {
    return {
      category: 'aggressive',
      description: 'reminiscent of Bobby Fischer and Magnus Carlsen - aggressive players who seek dynamic positions',
...
function calculatePhaseAccuracies(games: any[]): DeepAnalysisData['phaseAccuracies'] {
  const analyzedGames = games.filter(g => g.opening_accuracy && g.middle_game_accuracy && g.endgame_accuracy)
  ...
  return {
    opening: Math.round(analyzedGames.reduce((sum, g) => sum + g.opening_accuracy, 0) / analyzedGames.length),
    middleGame: Math.round(analyzedGames.reduce((sum, g) => sum + g.middle_game_accuracy, 0) / analyzedGames.length),
    endgame: Math.round(analyzedGames.reduce((sum, g) => sum + g.endgame_accuracy, 0) / analyzedGames.length)
  }
}
...
function identifyPrimaryStrengths(scores: DeepAnalysisData['personalityScores']): string[] {
  ...
  if (scores.endgame >= 70) strengths.push('Endgame technique')
  if (scores.opening >= 70) strengths.push('Opening knowledge')
  return strengths
}
...
function identifyImprovementAreas(scores: DeepAnalysisData['personalityScores']): string[] {
  ...
  if (scores.endgame < 50) areas.push('Endgame technique')
  if (scores.opening < 50) areas.push('Opening theory')
  return areas
}

function generatePlayingStyleDescription(style: DeepAnalysisData['playerStyle'], scores: DeepAnalysisData['personalityScores']): string {
...
function generateRecommendations(scores: DeepAnalysisData['personalityScores'], improvementAreas: string[]): DeepAnalysisData['recommendations'] {
...
function getDefaultAnalysis(): DeepAnalysisData {
  return {
    ...
    averageAccuracy: 0,
    currentRating: 0,
    personalityScores: { tactical: 50, positional: 50, aggressive: 50, patient: 50, endgame: 50, opening: 50 },
