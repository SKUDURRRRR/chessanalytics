...
export function AccountSetupModal() {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
      <div className="max-w-lg w-full rounded-3xl border border-white/10 bg-slate-900 p-8 shadow-2xl shadow-black/50">
        {/* Header */}
        <div className="flex items-start justify-between mb-6">
          <div>
            <h2 className="text-2xl font-bold text-white mb-2">Connect your chess account</h2>
        ...
        <div className="flex items-center justify-between">
          ...
          {(chessComLinked || lichessLinked) && (
            <button
              onClick={handleClose}
              disabled={closing}
              className="rounded-xl bg-emerald-500 hover:bg-emerald-600 px-6 py-2.5 text-sm font-semibold text-white transition-colors disabled:opacity-50"
            >
              {closing ? 'Saving...' : 'Done'}
            </button>
          )}
