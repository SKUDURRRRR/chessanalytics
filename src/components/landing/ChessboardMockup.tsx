    ...
    <div className="text-white" style={{ fontSize: '10px' }}>
      ...
      <div className="flex gap-5 p-5">
        ...
        <div className="shrink-0">
          <div className="flex gap-1.5">
            ...
            <div className="relative overflow-hidden rounded" style={{ width: 16, height: boardPx }}>
              <div className="absolute inset-x-0 top-0 bg-surface-2" style={{ height: '45%' }} />
          ...
          <div className="text-center mt-2.5">
            <div className="text-[7px] text-gray-500 mb-1">Use &larr; &rarr; arrow keys or click buttons to navigate</div>
            <div className="flex items-center justify-center gap-1.5">
              {['<<', '<', '>', '>>'].map(b => (
                <span key={b} className="rounded shadow-card bg-white/10 px-2 py-0.5 text-[8px] text-gray-400">
                  {b}
                </span>
              ))}
            </div>
        ...
        <div className="flex-1 min-w-0">
          <div className="rounded-lg shadow-card bg-white/[0.04] p-4 h-full flex flex-col">
            {/* Current Move */}
            <div className="text-[8px] font-semibold uppercase tracking-wide text-gray-500">Current Move</div>
            <div className="flex items-center gap-2 mt-1.5">
              <span className="text-xl font-semibold text-white leading-none">h6</span>
              <span className={`rounded-full px-2 py-0.5 text-[7px] font-semibold ${badgeColor['excellent']}`}>
                Excellent
              </span>
            </div>

            {/* Move Analysis */}
            <div className="mt-3 bg-surface-2/60 rounded-lg p-3 border-l-2 border-sky-400">
            ...
            {/* Follow-Up */}
            <button className="mt-2.5 w-full rounded-lg shadow-card bg-emerald-500/10 py-1.5 text-[8px] font-medium text-emerald-300">
              &rarr; Show Follow-Up
            </button>
            ...
            {/* Move Timeline */}
            <div className="text-[8px] font-semibold uppercase tracking-wide text-gray-500">Move Timeline</div>
            <div className="text-[6px] text-gray-500 mt-0.5 mb-2">&larr; &rarr; to navigate moves</div>

            <table className="w-full table-fixed text-left">
              <thead>
                <tr className="text-[7px] uppercase text-gray-500">
              ...
              <tbody>
                {moves.map(m => (
                  <tr key={m.num} className="border-b border-white/5 last:border-b-0">
                    <td className="py-1 text-[8px] text-gray-500">{m.num}</td>
                    <td className="py-1 pr-1.5">
                      <div className="flex items-center justify-between rounded-lg px-2 py-1 bg-white/[0.06]">
                        <span className="text-[8px] font-medium text-gray-300">{m.you}</span>
                        <span className={`rounded-full px-1.5 py-0.5 text-[6px] font-semibold ${badgeColor[m.youClass]}`}>
                    ...
                    <td className="py-1 pl-1.5">
                      <div className="flex items-center justify-between rounded-lg px-2 py-1 bg-white/[0.06]">
                        <span className="text-[8px] font-medium text-gray-300">{m.opp}</span>
                        <span className={`rounded-full px-1.5 py-0.5 text-[6px] font-semibold ${badgeColor[m.oppClass]}`}>
      ...
      <div className="border-t border-white/10 mx-5 pt-4 pb-5">
        ...
        <div className="text-[7px] text-gray-500 mt-0.5">Comprehensive analysis with actionable insights</div>
        ...
        <div className="grid grid-cols-4 gap-2.5 mt-3">
          {[
            ...
            <div key={stat.label} className="rounded-lg shadow-card bg-white/[0.04] p-2.5">
              <div className="text-[11px] font-semibold text-emerald-400">{stat.value}</div>
              <div className="text-[6px] text-gray-500 mt-1 leading-tight">{stat.label}</div>
            </div>
          ))}
        </div>
      </div>
