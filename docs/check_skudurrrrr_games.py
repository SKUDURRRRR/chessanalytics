#!/usr/bin/env python3
"""
Script to check what games are available for skudurrrrr user.
"""

import requests
import json

def check_skudurrrrr_games():
    """Check what games are available for skudurrrrr."""
    
    print("🔍 Checking games for skudurrrrr...")
    print("=" * 50)
    
    # Check for games in the database for skudurrrrr
    try:
        # Try to get some games from the database
        url = "http://localhost:8002/api/v1/analyses/skudurrrrr/lichess"
        ...
        if response.status_code == 200:
            data = response.json()
            print(f"✅ Found {len(data)} games in database for skudurrrrr")
            ...
            if data:
                ...
                for i, game in enumerate(data[:5]):  # Show first 5 games
                    ...
                    print(f"     Opponent: {game.get('opponent_name', 'N/A')}")
                    print()
                ...
                print("❌ No games found in database for skudurrrrr")
                return None
        else:
...
def test_reanalysis_skudurrrrr(game_id):
    """Test re-analysis with a real game from skudurrrrr."""
    ...
    test_data = {
        ...
        "user_id": "skudurrrrr",  # Correct user ID
        "platform": "lichess",
        "game_id": game_id,
        "depth": 8,
        "skill_level": 8
    ...
    print(f"\n🧪 Testing Re-analysis with skudurrrrr's game: {game_id}")
    print("=" * 50)
...
if __name__ == "__main__":
    # First check what games are available for skudurrrrr
    game_id = check_skudurrrrr_games()
    
    # Then test re-analysis with a real game
    if game_id:
        test_reanalysis_skudurrrrr(game_id)
    else:
        print("\n💡 No games found in database for skudurrrrr. You may need to:")
