...
def calc_novelty_game(total, unique, most_common_count):
    ...
    base = 45.0
    diversity_bonus = diversity_score * 0.35
...
def calc_staleness_game(total, unique, most_common_count):
    ...
    base = 50.0
    repetition_bonus = repetition_ratio * 90.0
    diversity_penalty = diversity_score * 0.2
...
for name, total, unique, most_common, move_nov, move_stal, target_nov, target_stal in test_cases:
    ...
    print(f"  Move-level: Nov {move_nov}, Stal {move_stal}")
    ...
    status = "✅ In range!" if nov_ok and stal_ok else f"{'⚠️ Nov' if not nov_ok else ''} {'⚠️ Stal' if not stal_ok else ''}"
    print(f"     {status}")
